...
class evr_namer:
    ...
    def get_date_and_time(self, path):
        ...
        try:
            os.rename(self.filename, self.filename.replace(self.old_filename, self.old_filename + '_' + self.new_filename))
        except:
            fileno+= 1
            self.new_filename = self.old_filename + '_' + self.new_filename + '_' + str(fileno)

def run_naming():
    fileobj = evr_namer()
    Voice_files_folder = os.path.join(os.getcwd(), "Voice_files_folder")
    print(Voice_files_folder)
    for file_name in os.listdir(Voice_files_folder):
        fileobj.filename = os.path.join(Voice_files_folder,  file_name)
